...
# to match ggsave dpi; showtext by default uses 350
...
df_team_logo_espn <- espn_nba_teams() %>%
  ...
  mutate(display_name = recode(display_name, !!!recode_key),
...
ggplot(data = df_shots, aes(x = team_season, y = abbreviation,
                            fill = x16_3p,
                            label = rank)) + 
  scale_fill_stepsn(colors = met.brewer("Tam"),
                    n.breaks = 10,
                     ...
                     guide = guide_coloursteps(frame.colour = "black",
                                             ...
                                             title.hjust = .5)) + 
  geom_tile(size = .1) + 
  geom_tile(data = df_shots %>% filter(rank < 11),
            color = "black",
            fill = NA,
            size = .8) +
  ...
  labs(title = "The Decline of the Midrange Jumper", 
       x = "", y = "",
  ...
  theme(axis.text.y = element_text(hjust = 1, vjust = 0.5), 
        ...
        legend.title = element_text(size = 25, color = "#36454F", 
                                    margin = margin(t = 0)),
        legend.margin = margin(t = 5, b = 5),
        plot.title = element_text(family = "Anton", size = 55, margin = margin(b = 10),
                                  color = "#36454F"), 
        plot.subtitle = element_textbox_simple(size = 27, margin = margin(b = 15), lineheight = .4,
                                               color = "#36454F"),
