...
df_count_threes_teams %>% 
  ...
  scale_x_continuous(labels = c("'08", "'21"),
                     breaks = c(2008, 2021)) + 
  labs(
    title = "What % of NBA games per year does a team attempt more 3-pt than 2-pt shots?",
  ...
  theme(
    ...
    strip.text = element_textbox(
      ...
      r = unit(7, "pt"),
      width = unit(1, "npc"))) +
  ggsave(here::here("threes_twit.png"), width = 15, height = 9)


```

```{r}
animate_df_count <- df_count_threes_teams %>% 
  ggplot(aes(x = year_season, y = perc, color = name_team)) + 
  geom_line(size = 1.1) +
  geom_point(size = 1.2) + 
  gghighlight(use_direct_label = FALSE, use_group_by = FALSE) + 
